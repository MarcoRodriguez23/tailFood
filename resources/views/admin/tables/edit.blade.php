              ...
              <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                  <form method="POST" action="{{route('admin.tables.update',$table->id)}}">
                    ...
                    <div class="sm:col-span-6">
                      ...
                      <div class="mt-1">
                        <input type="text" id="name" name="name" value="{{$table->name}}" class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal sm:text-sm sm:leading-5" />
                      </div>
                    </div>
                      ...
                      <div class="mt-1">
                        <select id="status" name="status" class="form-multiselect block w-full mt-1">
                          ...
                          @foreach (App\Enums\TableStatus::cases() as $case)
                            <option value="{{$case->value}}" @selected($table->status->value == $case->value)>{{$case->name}}</option>
                          @endforeach  
                        </select>
                      </div>
                    </div>
                      ...
                      <div class="mt-1">
                        <select id="location" name="location" class="form-multiselect block w-full mt-1">
                          ...
                          @foreach (App\Enums\TableLocation::cases() as $case)
                            <option value="{{$case->value}}" @selected($table->location->value == $case->value)>{{$case->name}}</option>
                          @endforeach  
                        </select>
                      </div>
                    </div>
